#!/usr/bin/env python3
"""
GPT Image Generator Tool Server Runner

This module provides the entry point for starting the GPT Image Generator Tool server.
...
SYSTEM_PROMPT = """
You have access to GPT-powered image generation tools. Key capabilities:
- Generate new images from text prompts (create_image_with_gpt)
- Edit/combine existing images based on text prompts (edit_image_with_gpt)

Basic usage:
- For new images: Provide save_path and detailed prompt
- For image editing: Provide save_path, prompt, and reference image paths
- Both tools return the path where the generated image is saved

Limitations:
- Requires valid image paths for editing tool
- Output quality may vary based on prompt specificity
...
async def create_agent(use_ai_server=False):
    ...
    # Create a new agent
    agent = Agent('gpt_image_generator_agent')
    ...
    agent.add_tool(tool_path, 'gpt_image_generator')
...
async def main():
    ...
    print(f"Starting GPT Image Generator Tool in {mode_type} mode using {server_type} tools.")
