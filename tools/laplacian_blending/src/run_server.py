#!/usr/bin/env python3
"""
Image Blender Tool Server Runner

This module provides the entry point for starting the Image Blender Tool server.
...
SYSTEM_PROMPT = """
You can blend two images using Laplacian pyramid blending with a mask. Key capabilities:  
- Blend images from URLs or local paths  
- Adjust blending levels for smoother transitions  
- Automatic resizing to match image dimensions  
- Save output to specified directory  

Basic usage: Provide two image paths/URLs and a mask path. The mask determines blending areas (white=image A, black=image B). Optional parameters: output directory and pyramid levels (default=6).  

Limitations: All inputs must be valid images. Mask should be grayscale (converted automatically if color). Output format is always PNG.
"""

async def create_agent(use_ai_server=False):
    """Create and initialize the Agent with appropriate tools"""
    # Create a new agent
    agent = Agent('image_blender_agent')
    ...
    agent.add_tool(tool_path, 'image_blender')
...
async def interactive_mode(agent):
    ...
    print("\nImage Blender Tool Interactive Mode")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description='Run Image Blender Tool Server')
    ...
    print(f"Starting Image Blender Tool in {mode_type} mode using {server_type} tools.")
