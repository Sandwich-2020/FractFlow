...
## 阶段4：物体位置评估
- 调用SupportRelationTool,对输入的3D场景JSON文件，判断物体支撑关系是否合理，判断物体是否存在物理关系异常
- 对物体支撑关系输出从floating， unsupported，overlap_door，overlap_window四个维度的分析
- 解析返回结果保存，并提取路径列表
...
## 阶段6：结构化输出结论
- 从物体尺寸评估, 支撑关系评估, 场景评估保存路径提取评估结论进行结构化分析
- 物体尺寸评估,支撑关系评估, 场景评估均以结构化YAML输出
- 根据物体尺寸评估, 支撑关系评估, 场景评估三个维度评估结论生成总评估（answer, confidence, Success)
...
2. **SupportRelationTool** - 物体位置评估
   - 输入：渲染图像路径 + 结构化评估提示
   - 输出：位置评估结果（包含answer, visual_reference, success）

3. **SceneEvaluationTool** - 场景级质量评估
   - 输入：渲染图像路径 + 结构化评估提示
   - 输出：场景评估结果（包含error, message, success）
...
support_relation_evaluation:
  errors:
    ...
    - object_id: "cabinet_1"
      ...
      desc: "cabinet_1 距离窗 window_3 仅 0.80m 且顶部高于1.7m，可能影响采光。"
    # ... 其他错误 ...
  summary:
    message: "共检测到3处物理关系异常。"
  success: true/false

scene_evaluation:
  answer:
    对象计数:
      message: "对象计数评估结论（含百分比值）"
      error: "如有错误或不符点，列出具体问题，无则为null"
      fidelity: 0.0  # 匹配度分数，0-1
    对象属性:
      message: "对象属性评估结论（含百分比值）"
      error: "如有错误或不符点，列出具体问题，无则为null"
      fidelity: 0.0
    对象-对象关系:
      message: "对象-对象关系评估结论（含百分比值）"
      error: "如有错误或不符点，列出具体问题，无则为null"
      fidelity: 0.0
    对象架构关系:
      message: "对象架构关系评估结论（含百分比值）"
      error: "如有错误或不符点，列出具体问题，无则为null"
      fidelity: 0.0
    文本场景描述一致性:
      message: "与文本描述逐条核查的结论（含匹配度/主要不符点）"
      error: "如有错误或不符点，列出具体问题，无则为null"
      fidelity: 0.0
  Conclusion: "整体评估结论"
  fidelity: 0.0  # 整体匹配度分数，0-1
  success: true/false
...
- 输出必须简洁、客观、结构化
    ...
    TOOLS = [
        ...
        ("tools/core/QualityEvaluation/Support_relation_agent.py", "Support_relation_detection"),
    ]
...
3. **Support Relation Analysis**: Object support relation evaluation and assessment  
...
- json_path: Path to 3D scene JSON file
...
- Object support relation appropriateness
