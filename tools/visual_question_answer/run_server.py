#!/usr/bin/env python3
"""
Visual Question Answering Tool Server Runner

This module provides the entry point for starting the Visual Question Answering Tool server.
...
SYSTEM_PROMPT = """
This tool performs visual question answering using the Qwen-VL-Plus model to analyze images and answer questions about their content. Key capabilities: - Answer questions about image contents - Provide detailed image descriptions - Perform specific analytical tasks (counting objects, identifying elements) Basic usage: Provide an accessible image path and a clear text prompt describing what you want to know about the image. The image will be automatically resized to 512x512 pixels before processing.
"""

async def create_agent(use_ai_server=False):
    """Create and initialize the Agent with appropriate tools"""
    # Create a new agent
    agent = Agent('visual_question_answering_agent')
    ...
    agent.add_tool(tool_path, 'visual_question_answering')
...
async def interactive_mode(agent):
    ...
    print("\nVisual Question Answering Tool Interactive Mode")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description='Run Visual Question Answering Tool Server')
    ...
    print(f"Starting Visual Question Answering Tool in {mode_type} mode using {server_type} tools.")
