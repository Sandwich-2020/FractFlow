...
class BaseToolCallingHelper(ABC):
    """
    Abstract base class for tool calling helpers.
    
    Defines common interface and implements shared functionality for
    generating tool calls across different LLM providers.
    """
    
    def __init__(self):
        """Initialize the base tool calling helper."""
    ...
    def create_system_prompt(self, tools: List[Dict[str, Any]]) -> str:
        """
        Create a system prompt for tool calling.
    ...
    async def _create_chat_completion(self, **kwargs) -> Any:
        """
        Handle API call to the provider.
        
        Args:
            **kwargs: Arguments to pass to the provider API
...
class DeepSeekToolCallingHelper(BaseToolCallingHelper):
    """
    Implementation of BaseToolCallingHelper for DeepSeek models.
    
    Handles DeepSeek-specific client initialization, prompting, and response parsing.
    """
    ...
    def __init__(self):
        ...
        super().__init__()
        self.client = None
        self.model = self.config.get('deepseek.tool_calling_model', 'deepseek-chat')
    ...
    async def initialize_client(self) -> OpenAI:
        ...
        Returns:
            Configured OpenAI client for DeepSeek
        """
        if self.client is None:
            self.client = OpenAI(
                base_url=self.config.get('deepseek.base_url', 'https://api.deepseek.com'),
    ...
    def create_system_prompt(self, tools: List[Dict[str, Any]]) -> str:
        """
        Create a system prompt for JSON tool calling with DeepSeek.
...
Output JSON only, no other text. The arguments must be a valid JSON string (with escaped quotes)."""
    ...
    async def _create_chat_completion(self, **kwargs) -> Any:
        ...
        try:
            ...
            logger.error(f"DeepSeek API call error: {error}")
            return None
    
    async def _parse_model_response(self, response: Any) -> Optional[List[Dict[str, Any]]]:
        """
        Parse the response from DeepSeek into a list of tool calls.
        
        Args:
            response: Raw response from the DeepSeek model
...
class QwenToolCallingHelper(BaseToolCallingHelper):
    """
    Implementation of BaseToolCallingHelper for Qwen models.
    
    Handles Qwen-specific client initialization, prompting, and response parsing.
    """
    ...
    def __init__(self):
        ...
        super().__init__()
        self.client = None
        self.model = self.config.get('qwen.tool_calling_model', 'qwen-max')
    ...
    async def initialize_client(self) -> OpenAI:
        ...
        Returns:
            Configured OpenAI client for Qwen
        """
        if self.client is None:
            self.client = OpenAI(
                base_url=self.config.get('qwen.base_url', 'https://dashscope.aliyuncs.com/compatible-mode/v1'),
    ...
    def create_system_prompt(self, tools: List[Dict[str, Any]]) -> str:
        """
        Create a system prompt for JSON tool calling with Qwen.
...
Output JSON only, no other text. The arguments must be a valid JSON string (with escaped quotes)."""
    ...
    async def _create_chat_completion(self, **kwargs) -> Any:
        ...
        try:
            ...
            error = handle_error(e, {"kwargs": kwargs})
            logger.error(f"Qwen API call error: {error}")
            return None
    
    async def _parse_model_response(self, response: Any) -> Optional[List[Dict[str, Any]]]:
        """
        Parse the response from Qwen into a list of tool calls.
        
        Args:
            response: Raw response from the Qwen model
        ...
        try:
            ...
            # Handle other Qwen-specific formats here if needed
...
class ToolCallingHelperFactory:
    """
    Factory class for creating appropriate ToolCallingHelper instances.
    
    Uses configuration to determine which helper implementation to use.
    Falls back to default helper matching the model provider.
    """
    
    _helper_classes: Dict[str, Type[BaseToolCallingHelper]] = {
        'deepseek': DeepSeekToolCallingHelper,
        'qwen': QwenToolCallingHelper,
    }
    
    @classmethod
    def create_helper(cls, provider: Optional[str] = None) -> BaseToolCallingHelper:
        """
        Create a tool calling helper for the specified provider.
        ...
        Returns:
            Configured ToolCallingHelper implementation
        """
        config_manager = ConfigManager()
        
        # If provider is not specified, use configuration
        if not provider:
            # Get the specific tool_calling provider from config
            provider = config_manager.get('tool_calling.provider')
            
            # If tool_calling.provider is not set, use the main model provider
            if not provider:
                provider = config_manager.get('agent.provider')
        
        # Normalize provider name
        provider = provider.lower() if provider else 'deepseek'
        
        # Get the appropriate helper class
        helper_class = cls._helper_classes.get('deepseek')
        
        # # If not found, log warning and use default
        # if not helper_class:
        #     logger.warning(f"No tool calling helper found for provider '{provider}', using default")
        #     helper_class = cls._helper_classes.get('deepseek', DeepSeekToolCallingHelper)
            
        # # Create and return the helper
        # logger.debug(f"Creating tool calling helper for provider '{provider}'")
        return helper_class()
