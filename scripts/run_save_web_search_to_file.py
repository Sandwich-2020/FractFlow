...
async def create_agent():
    ...
    config['agent']['custom_system_prompt'] = '你是一个果断的执行家，你将根据用户的需求，使用web_search工具和web_browse工具来完成任务，请直接执行，不要询问用户是否需要进一步操作。'
    ...
    agent.add_tool("./tools/websearch/src/server.py", "websearch_tool")
    agent.add_tool("./tools/file_io/src/server.py", "fileio_tool")
